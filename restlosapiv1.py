...
from utils.ErrorHandler.RequestHandler import  geterrorhandlerdetails,rdfile,wrtfile
...
#app = Flask(__name__)
config = Config(os.path.join(os.path.dirname(__file__), 'configv1.json'))
VERSION="1"
...
class NagiosControlView(MethodView):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            self.command_file = Command.find_command_file(config['nagios_main_cfg'])
#            self.command_file = Model.Control.Command.find_command_file(config['nagios_main_cfg'])
    ...
    def _restart(self):
        ...
        try:
            d = daemon(nagios_bin=config['nagios_bin'], nagios_cfg=config['nagios_main_cfg'],
                       nagios_init=config['nagios_init'],sudo=True, service_name=config['service_name'])
...
#        self.add_url_rule('/errorhandler/<handler>/<subhandler>', view_func=errorhandlerView.as_view('errorhandler'))
#        self.add_url_rule('/errorhandler', view_func=errorhandlerView.as_view('geterrorhandler'))
...
############## Code for the Error Handler Management ###############################
