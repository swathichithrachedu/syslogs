"""
This code work as controller which control the RESTAPI request.
In this mostly it deals with nagios configuration.
it also deal with verify and restart nagios server
"""
...
from pynag.Control import Command, daemon, PynagError
from werkzeug.exceptions import HTTPException, InternalServerError
from werkzeug.exceptions import default_exceptions, BadRequest

from adapters.utils import Config
from adapters.utils.authentication import Authentify
from loggers.sealfuncApp import syslog
config = Config(os.path.join(os.path.dirname(__file__),
                             'configurations/nagios/configv1.json'))
VERSION = "1"
...
class JSONHTTPException(HTTPException):
    ...
    def get_body(self, environ):
        logging.error("In Get Body: "+self.get_description(environ))
...
class JSONBadRequest(JSONHTTPException, BadRequest):
    description = "The browser (or proxy) sent a request that this server " \
...
class ApiEndpoints(dict):
    ...
    def __init__(self):
        ...
        for endpoint in Model.all_attributes.object_definitions.keys():
            self[endpoint] = Model.all_attributes.object_definitions[
                endpoint].keys()
    ...
    def validate(self, endpoint, data={}):
        for attr in data.keys():
            ...
            if attr == self.endpoint_keys[endpoint]:
                for illegal_char in self.main_cfg_values.get(
                        'illegal_object_name_chars', []):
                    if illegal_char in list(data[attr]):
                        if illegal_char == '*':
                            continue  # wildcards are allowed & will be
                            # stripped out
                        return {
                            400: "illegal character (%s) found in attribute "
                                 "%s" % (illegal_char, attr)}
        return {200: "OK"}
...
class NagiosControlView(MethodView):
    ...
    def __init__(self, *args, **kwargs):
        syslog('', ' * starting server V%s' % (VERSION,), '', '')
        MethodView.__init__(self, *args, **kwargs)

        try:
            self.command_file = Command.find_command_file(
    ...
    def _format(self, data):
        result = {'Error': [], 'Warning': [], 'Total Errors': [],
                  'Total Warnings': []}
    ...
    def _verify(self):
        try:
            if config['sudo']:
                output = check_output(['sudo', config['nagios_bin'], '-v',
                                       config['nagios_main_cfg']])
            else:
                print [config['nagios_bin'], '-v', config['nagios_main_cfg']]
                output = check_output(
                    [config['nagios_bin'], '-v', config['nagios_main_cfg']])
            returncode = 0
        ...
        return {'output': result if result else output,
                'returncode': returncode}

    def _restart(self):
        logging.warn("[audit] [user: %s] triggered the restart "
                     "command" % (request.authorization.username), )
        try:
            d = daemon(nagios_bin=config['nagios_bin'],
                       nagios_cfg=config['nagios_main_cfg'],
                       nagios_init=config['nagios_init'], sudo=True,
...
class NagiosObjectView(MethodView):
    ...
    def get(self):
        ...
        try:
            result = [obj['meta']['defined_attributes'] for obj in
                      endpoint_objects.filter(**query)]
        except IOError, err:
    ...
    def delete(self):
        ...
        for obj in objects:
            try:
                ...
                results.append({500: "unable to delete %s object %s: "
                                     "%s" % (self.endpoint,
                                             obj[unique_key], str(err))})
                logging.debug(
                    "[audit] [user: %s] failed to delete %s object %s: %s" % (
                        self.username, self.endpoint, obj[unique_key],
                        str(err)))
            else:
                results.append({200: "successfully deleted %s object: "
                                     "%s" % (self.endpoint, obj[unique_key])})
                logging.info(
                    "[audit] [user: %s] deleted %s object: "
                    "%s" % (self.username, self.endpoint, obj[unique_key]))

        summary = self._summary(results)
        logging.warn(
            "[audit] [user: %s] deleted %d %s objects "
            "(out of %d requested)" % (
    ...
    def post(self):
        ...
        if data is None:
            return jsonify(
                message='no json received. you need to set your content-type to application/json.')
        ...
        summary = self._summary(results)
        logging.warn(
            "[audit] [user: %s] stored %d %s objects (out of %d requested)" % (
                self.username,
                summary['succeeded'],
                self.endpoint,
                summary['total'])
    ...
    def _save_or_update(self, item):
        ...
        if unique_key in item.keys():
            ...
            try:
                endpoint_object = getattr(
                    Model, self.endpoint.capitalize()).objects.filter(**query)
            except IOError, err:
                abort(500, "error opening config files: %s" % (str(err),))
            except:
                abort(500)
            ...
            return {500: 'required key for %s object not set: '
                         '%s' % (self.endpoint, unique_key)}
        ...
        try:
            ...
            logging.debug(
                "[audit] [user: %s] failed to store %s object %s: %s" % (
                    self.username, self.endpoint, item[unique_key], str(err)))
            return {500: 'unable to save %s object %s: '
                         '%s' % (self.endpoint, item[unique_key], str(err))}

        logging.info("[audit] [user: %s] stored %s object "
                     "%s" % (self.username, self.endpoint, item[unique_key]))
        return {200: "successfully stored %s object: "
                     "%s" % (self.endpoint, item[unique_key])}
    ...
    def __register_endpoints(self):
        for endpoint in self.endpoints.keys():
            self.add_url_rule('/' + endpoint,
                              view_func=NagiosObjectView.as_view(endpoint))
        self.add_url_rule('/control',
                          view_func=NagiosControlView.as_view('control'))
...
if __name__ == '__main__':
    ...
    logging.info(" * starting server V%s" % (VERSION,))
    syslog('', ' * starting server V%s' % (VERSION,), '', '')
